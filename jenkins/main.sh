...
# Parse arguments
# The first argument is 'install' or 'update'
# If 'install', the remaining arguments are paths to one or more tool request files in shed-tools format
# If 'update', no more arguments are parsed
MODE="$1"; # Two modes possible: "install" for tool request, "update" for cron update
ARGS=( "$@" )
FILE_ARGS=("${ARGS[@]:1}")
LOG_DIR=~/galaxy_tool_automation  # PUT THIS IN .ENV
...
# no need to export if the scope has changed and everything is running here, will this work?
...
if [ $MODE = "install" ]; then
  ...
  # no need to export anything now?
